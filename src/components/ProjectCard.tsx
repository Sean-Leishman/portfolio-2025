...
function ProjectCard({ name, date, description, blogLink, externalLink, githubLink, image, technologies }: { name: string, date: string, description: string, blogLink: string | undefined, externalLink: string | undefined, githubLink: string, image?: string, technologies: string[] }) {
    ...
    if (blogLink) {
        githubIcon = <span className="flex items-center gap-2 mr-2">{githubIcon} | </span>;
    }
    else {
    ...
    return (
        <div className="flex flex-col gap-3 mb-10 text-sm justify-start w-full">
            <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center gap-4">
                    <div className="flex justify-start items-center gap-2">
                        <h3 className="text-lg font-extrabold text-foreground">{name}
            ...
            <div className="flex flex-col gap-2">
                {image && <Figure src={image} alt={`${name} screenshot`} className="w-full h-64 object-cover rounded-md" />}
